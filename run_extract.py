...
if __name__ == '__main__':
    ...
    hidden_size = np.size(points) * 2

    # 网络建立
    net_model = model.model(model_name=model_name, input_size=input_size, hidden_size=hidden_size)
    net_model.net_set(save_name, device_name=device_name, field_name=field_name, field_bound=field_bound,
    ...
    # 数据载入
    test_loader = loader.get_test_loaders(test_dir=test_dir, field_bound=field_bound, batch_size=1, points=points)

    net_model.load_model(resume_pattern=None, model_name="lowest_loss_model.pth")

    net_model.feature_extract(test_loader)
